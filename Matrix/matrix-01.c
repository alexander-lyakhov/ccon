...
Drop Drop_create(Console *console)
{
	...
	if ((rand() % 101) > 90) {
		attr = 0x0a;
		count = (rand() % console->height >> 1) + 3; // min length
	}
	else {
		attr = (rand() % 101) > 90 ?  0x02 : 0x00;
		count = (rand() % console->height / 3) + 3; // min length
	}
	...
	for (int i = 0; i < console->width; i++)
	{
		...
		else
		{
			...
			*/
			if (console->attrs[i] != ATTR_BLACK && console->attrs[i + console->width] == ATTR_BLACK)
			{
				console->attrs[i] = console->attrs[i] & 8 ? ATTR_WHITE : 0x07;
			}
		}
